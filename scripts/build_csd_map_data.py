# -*- coding: utf-8 -*-
"""
Build the data payload for the municipality-level (Census Subdivision)
choropleth map — same approach as build_fsa_map_data.py, different
geographic unit.
"""
import csv
import json
...
GEOJSON_PATH = BASE / "data" / "ontario_csd_simplified.geojson"
STATS_PATH = BASE / "data" / "csd_applications_normalized.csv"
OUT_PATH = BASE / "data" / "csd_map_payload.json"
    ...
    with open(STATS_PATH, encoding="utf-8") as f:
        for row in csv.DictReader(f):
            ...
            stats[row["csduid"]] = {
                ...
                "total": float(row["total_applications"]),
                "landlord": float(row["landlord_filed"]),
                "tenant": float(row["tenant_filed"]),
                "fsa_count": float(row["fsa_count"]),
...
def main():
    ...
    for feat in fc["features"]:
        ...
        total_for_weight = s["total"] if s else 0
        if total_for_weight > 0:
            ...
            centroid_weights.append(total_for_weight)

        features_out.append({
            "csduid": csduid,
            "name": name,
            "d": path_d,
            **({k: v for k, v in s.items() if k != "name"} if s else
               {"population": None, "total": 0, "landlord": 0, "tenant": 0, "fsa_count": 0,
    ...
    payload = {"width": width, "height": height, "focusView": focus_view, "features": features_out}
    with open(OUT_PATH, "w", encoding="utf-8") as f:
        json.dump(payload, f, separators=(",", ":"))

    print(f"Projected {len(features_out)} municipality polygons into a {width}x{height} viewBox")
    print(f"Matched to application stats: {matched}, unmatched (grey, no data): {unmatched}")
    print(f"Default focus viewport: {focus_view}")
