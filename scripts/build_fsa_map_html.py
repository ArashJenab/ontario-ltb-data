...
<div class="page">
  ...
  <div>
    ...
    <p class="dek">Landlord and Tenant Board applications by postal FSA (Forward Sortation Area, the first
       3 characters of a postal code, e.g. <b>N6J</b>), normalized against resident population.
       Toggle between raw volume and per-10,000-resident rates, and between landlord-filed and tenant-filed cases:
  ...
  <div class="layout">
    <div class="map-card">
      ...
      <div class="legend">
        ...
        <div class="legend-note" id="legend-note">Colored by value on a smooth scale: every area gets its own shade, so nearby values look similar but rarely identical.</div>
      </div>
  ...
  <footer>
    ...
    Raw counts favor high-population areas; per-10,000 rates get noisy for small-population FSAs, so treat single-digit-population-thousands
    FSAs with some caution. Grey areas have an FSA boundary but no matching LTB applications in this export.
    This is an independent analysis developed using data published by the Government of Ontario. It is not an
    official publication of, and is not affiliated with or endorsed by, the Government of Ontario or the
...
(function () {
  ...
  // one shared green-to-red ramp (low = green/fewer disputes, high = red/more
  // disputes) instead of a different hue per lens, using ColorBrewer's RdYlGn,
  // reversed so green sits at the low end.
  var GREEN_RED_RAMP = ["#1a9850", "#66bd63", "#a6d96a", "#fee08b", "#f46d43", "#a50026"];
  var RAMPS = { total: GREEN_RED_RAMP, landlord: GREEN_RED_RAMP, tenant: GREEN_RED_RAMP };
  ...
  // A smooth gradient across the ramp, not discrete bins, so two nearby-but-
  // different areas get visibly different shades instead of identical ones.
  // Uses a sqrt transform, not linear: this data is heavily right-skewed (a
  // couple of tiny-population FSAs post rates in the hundreds/10k while the
  // typical FSA sits under 30/10k), so a linear scale would leave almost
  // everything the same pale shade with one or two outliers eating the dark
  // end.
  ...
  function colorForFeature(f, lens, metric, ramp, domain) {
    var v = valueFor(f, lens, metric);
    if (v === null || v === undefined || !isReliable(f, metric)) return null;
    return continuousColor(ramp, v, domain);
  }
  ...
  function render() {
    ...
    var maxVal = domain.max;
    var colorOf = function (f) { return colorForFeature(f, lens, metric, ramp, domain); };
    ...
    document.getElementById('legend-title').textContent =
      LENS_LABEL[lens] + ' applications' + (metric === 'per10k' ? ' / 10,000 residents' : ' (raw count)') + ', by value';
