...
<div class="page">
  <div>
    ...
    <p class="dek">Landlord and Tenant Board applications by postal FSA, normalized against resident population.
       ...
       hover any area for the full breakdown.</p>
  </div>
...
(function () {
  ...
  function render() {
    ...
    pathEls.forEach(function (el, i) {
      ...
      if (v === null || v === undefined || !isReliable(f, metric)) {
        el.setAttribute("fill", getVar('--no-data'));
      } else {
        el.setAttribute("fill", ramp[binIndex(v, breaks)]);
      }
    });
    ...
    var rankHtml = ranked.map(function(f, i) {
      ...
      var barColor = ramp[ramp.length - 1];
  ...
  function wireToggle(id, key) {
    ...
    group.querySelectorAll('button').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        state[key] = btn.dataset[key === 'metric' ? 'metric' : 'lens'];
        render();
      });
    });
  }
